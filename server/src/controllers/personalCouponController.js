...
exports.myCoupons = async(req,res)=>{
 ...
 const now=new Date();
 const coupons=await prisma.serviceSale.findMany({where:{active:true,couponCode:{not:null},AND:[{OR:[{recipientAccountId:null,recipientAccountIds:{isEmpty:true}},{recipientAccountId:userId},{recipientAccountIds:{has:userId}}]},{OR:[{endsAt:null},{endsAt:{gt:now}}]}]},select:{id:true,title:true,couponCode:true,discountPercent:true,scope:true,serviceId:true,couponServiceIds:true,startsAt:true,endsAt:true,couponUses:{where:{accountId:userId,usedAt:{not:null}},select:{id:true}}},orderBy:{createdAt:'desc'},take:100});
