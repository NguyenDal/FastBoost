...
export default function DashboardCoupons(){
 ...
 </article>;})}</div>{copied==='failed'&&<p role="status">Select and copy the code manually.</p>}</div>{coupons.length > 0 && <OrderPagination currentPage={page} totalPages={totalPages} onPageChange={setPage} tableId="dashboard-coupon-list" label="Coupons pagination"/>}</section>;
}
